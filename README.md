...
- **�️ Real-Time Audio Vault**: Detects when a student joins a live meeting (Zoom, Teams). It proactively asks for consent to monitor outgoing audio, providing a real-time transcript overlay that chemically blurs spoken PII (SSNs, passwords) using a simulated Edge Speech-to-Text model.
- **👁️ Visual Shield OCR**: A local computer vision pipeline. Students can drop screenshots or ID photos into the vault. The local NPU sweeps the image, identifies text via OCR, maps bounding boxes over sensitive PII fields (DOB, Student ID), and redacts it with zero network overhead.
